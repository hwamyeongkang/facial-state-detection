...
class FacialStateGUI:
    def __init__(self, root=None):
        ...
        self.on_auto_music_toggle = None  # 음악 토글 콜백 추가
    
    def show_webcam_error(self):
        """웹캠 오류 메시지 표시"""
        try:
            # 기존 내용 제거
            for widget in self.webcam_display.winfo_children():
                widget.destroy()
    ...
    def setup_gui(self):
        ...
        # 음악 추천 섹션 추가 (control_panel에 직접 추가)
        self.control_frame = tk.Frame(control_panel, bg=GUI_COLORS['bg_mid'])
        self.control_frame.pack(fill=tk.X, padx=20, pady=10)
        
        # 음악 자동 추천 프레임과 토글 버튼
        self.music_frame = tk.LabelFrame(self.control_frame, text="음악 추천", padx=10, pady=5, 
                                       bg=GUI_COLORS['bg_mid'], fg=GUI_COLORS['text'])
        self.music_frame.pack(fill="x", padx=5, pady=5)

        self.auto_music_button = tk.Button(
            self.music_frame, 
            text="자동 음악 추천 켜기", 
            command=self.toggle_auto_music, 
            bg="#4CAF50", 
            fg="white"
        )
        self.auto_music_button.pack(fill="x", padx=5, pady=5)
...
- 불만족: 미간 거리 10% 이상 짧아짐 또는 입이 1cm 이상 올라감"""
    ...
    def setup_gui(self):
        ...
        # 음악 자동 추천 프레임과 토글 버튼
        self.music_frame = tk.LabelFrame(control_panel, text="음악 추천", padx=10, pady=5, 
                                       bg=GUI_COLORS['bg_mid'], fg=GUI_COLORS['text'],
                                       font=("Helvetica", 12, "bold"))
        self.music_frame.pack(fill=tk.X, padx=20, pady=5)
    ...
    def toggle_auto_music(self):
        """음악 자동 추천 토글 핸들러"""
        if self.on_auto_music_toggle:
            self.on_auto_music_toggle()    
    def set_auto_music_button_state(self, is_active):
        """음악 자동 추천 버튼 상태 업데이트"""
        if is_active:
            self.auto_music_button.config(text="자동 음악 추천 끄기", bg="#F44336")
        else:
            self.auto_music_button.config(text="자동 음악 추천 켜기", bg="#4CAF50")        